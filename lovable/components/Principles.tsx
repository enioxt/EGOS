...
export const Principles = () => {
  return (
    <section id="principles" className="py-16 bg-egos-pastel-green/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-egos-black">Guiding Principles</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 md:gap-8">
          {principles.map((principle, index) => (
            <div 
              key={index}
              className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow flex flex-col items-center text-center hover:translate-y-[-5px] transition-transform duration-300 animate-fade-in"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <principle.icon className="w-16 h-16 mb-4" />
              <h3 className="text-lg font-semibold mb-2 text-egos-plum">{principle.title}</h3>
