...
class ContractsService:
    ...
    def _validate_configuration(self) -> None:
        ...
        if not self.qdrant_url or not self.qdrant_api_key or not self.qdrant_collection_name:
    ...
    def _populate_collection(self) -> None:
        ...
        if points:
            try:
                ...
                logger.info(f"Upserted {len(points)} points from {files_processed} files")
            except Exception as e:
    ...
    def _semantic_chunker(self, text: str) -> List[str]:
        ...
        breakpoint_indices = [i for i, x in enumerate(distances) if x > breakpoint_threshold]
    ...
    def _process_contract(self, file_path: str, filename: str, md5: str) -> List[PointStruct]:
